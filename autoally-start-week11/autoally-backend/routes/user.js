...
// PUT - add favouriteService --------------------------------------
router.put('/addFavService/', Utils.authenticateToken, (req, res) => {  
  // validate check
  ...
  // add serviceId to favouriteServices field (array - push)
  User.updateOne({
    _id: req.user._id
  }, {
    $push: {
      favouriteServices: req.body.serviceId
    }
  })
    .then((user) => {            
      res.json({
    ...
    .catch(err => {
      console.log(err)
      res.status(500).json({
        message: "Problem adding favourite service"
      })
    })
})
...
router.get('/:id', Utils.authenticateToken, (req, res) => {
  ...
  User.findById(req.params.id).populate('favouriteServices')
    .then(user => {
      res.json(user)
    })
    .catch(err => {
